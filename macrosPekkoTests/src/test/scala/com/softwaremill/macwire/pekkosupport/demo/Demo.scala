...
object Demo extends App {
  ...
  val `4 voices polyphone` =
    system.actorOf(wireProps[PolyphoneActor].withRouter(RoundRobinPool(4))).taggedWith[Polyphone]
...
class PolyphoneActor extends Actor {
  override def receive: Receive = { case Play =>
    println(s"Polyphone ${self.path.name} is playing: uouiiiiiiiiiii")
  }
}
...
class ViolinActor(score: Score) extends Actor {
  override def receive: Receive = { case Play =>
    println("Violin is playing: uuuuuuu ...")
  }
}
...
class GuitarActor(score: Score) extends Actor {
  override def receive: Receive = { case Play =>
    println("Guitar is playing: pa pa pppaaa  ...")
  }
}
...
class SingerActor(score: Score) extends Actor {
  override def receive: Receive = { case Play =>
    println(s"${self.path.name} is singing: aaaaa uuuu eeee")
  }
}
