...
private[macwire] class CompanionCrimper[Q <: Quotes, T: Type](using val q: Q)(
  ...
  def wireParams(paramLists: List[List[Symbol]]): List[List[Term]] =
    paramLists.map(_.map(p => dependencyResolver.resolve(p, paramType(p))))
