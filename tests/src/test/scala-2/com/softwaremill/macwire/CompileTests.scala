...
class CompileTests extends CompileTestsSupport {
  ...
  runTestsWith(
    expectedFailures = List(
      "companionFactoryAmbiguous" -> List(
        "No public primary constructor found for",
        "Test.PrivConstructor",
      ...
      "explicitDepsWiredWithImplicitValsFromMethodScope" -> List(
        ambiguousResMsg("A"),
        "dependency",
        "implicitDependencyA"
      ),
    ...
    expectedWarnings = List(
      "forwardReferenceInBlock" -> List(
        "Found [a] for parameter [a], but a forward reference [forwardA] was also eligible"
      )
    )
  )
}
